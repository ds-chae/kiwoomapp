...
Log Viewer를 수정한다. 각각의 log file 이름 우측에 삭제 버튼을 생성한다. 사용자가 삭제 버튼을 클릭하면 해당 파일을 삭제한다.
...
images 버튼을 클릭해서 보여주는 image gallery 화면을 수정한다. 올림차순, 내림차순 정령을 할 때에 파일 명으로 하지 말고, 파일 명 중간에 나타나는 yyyymmdd 형식의 날짜와 시간으로 정렬하라. 파일명의 첫 저리는 C 또는 L이다. 두번재 자리부터 4자리는 ACCOUNT CODE 이다. 따라서 다섯째 자리부터 yyyymmdd가 나타날 가능성이 있는 것으로 분석한다.

autotr.py를 수정한다. UI의 Account 섹션의 Account 번호 우측에 "예수금" 버튼을 만들고, 이 버튼을 클릭하면 fn_kt00001.py에 있는 함수 get_yesu_list를 호출해서 "예수금" 버튼 우측에  entr(예수금), 
d1_entra(d+1추정예수금), d2_entra(d+2추정예수금)을 콤마로 자릿수를 구분하는 숫자를 보여준다. 이 숫자는 Account 번호마다 각각의 값을 저장해서 보여준다. 보여주는 형식은 Y=###, D+1=###, D+2=### 형식이다.

autotr.py 수정. 매수 금액과 평가 금액 컬럼을 Account holdings에 추가한다. "Account Holgins"는 "잔고"로 섹션 이름을 변경한다.

datetime.datetime과 datetime.time을 비교하는 문제를 수정하라.
