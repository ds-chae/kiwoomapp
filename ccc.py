...
def fn_kt00018(log_jango, token, data, cont_yn='N', next_key=''):
	...
	if log_jango:
		...
		print('get_jango => Header:',
			  json.dumps({key: response.headers.get(key) for key in ['next-key', 'cont-yn', 'api-id']}, indent=4,
						 ensure_ascii=False))
		print('get_jango => Body:',
			  json.dumps(response.json(), indent=4, ensure_ascii=False))  # JSON ÀÀ´äÀ» ÆÄ½ÌÇÏ¿© Ãâ·Â
	...
	for bal_rt in day_bal_rt:
		...
		if bal_rt['stk_nm'] == '박셀바이오':
			...
			if rmnd_qty != '0':
				...
				ret_status = sell_order(TOKEN, dmst_stex_tp='NXT', stk_cd=bal_rt['stk_cd'], ord_qty=rmnd_qty,
										ord_uv=ord_uv, trde_tp=trde_tp, cond_uv='')
...
def buy_cl(now, MY_ACCESS_TOKEN):
	...
	for stk_cd in interested_stocks:
		if stk_cd in order_count and order_count[stk_cd] >= 2:
			continue
...
def daily_work(now):
	...
	global nxt_start_time, nxt_end_time, nxt_fin_time, krx_start_time, nxt_cancelled, krx_end_time
...
async def buy_order_api(request: dict, proxy_path: str = "", token: str = Cookie(None)):
	...
	return {"status": "success", "message": f"Buy order placed: {ord_qty} shares of {stk_nm or stk_cd} at {ord_uv}",
			"data": ret_status}
...
async def set_auto_sell_api(request: dict, proxy_path: str = "", token: str = Cookie(None)):
	...
	try:
		...
		if save_auto_sell_to_json():
			return {"status": "success", "enabled": auto_sell_enabled[account],
					"message": f"Auto sell {'enabled' if auto_sell_enabled[account] else 'disabled'} for account {account}"}
		else:
